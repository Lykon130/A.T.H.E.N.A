...
**HUD layout — built.** Idle view (rotating neural mesh, department zones) + Ctrl+K command palette into per-department detail views (`MetricsPanel`, `FeedLogPanel`, `PipelinePanel`, `ReferencePanel`), not one flat dashboard. Lives in `hud/` (Electron + React + TypeScript), polls the vault via the Obsidian Local REST API. The speaking-reactive wave sweep is now driven by live voice activity: `hud/src/main/voiceBridge.ts` hosts a local WebSocket server (`127.0.0.1:8765`) that `voice/hud_bridge.py` streams real TTS playback amplitude to (~30Hz, measured off the actual Piper buffer during `sd.play()`), relayed to the renderer over IPC. Decoupled by design — the HUD works with no voice process connected (wave just stays idle), and `voice/` no-ops if the HUD isn't running. The old synthetic sine-blend envelope moved to `lib/syntheticVoice.ts`, still used by the manual Ctrl+K "speak" command bar test.

**Voice — built and live-tested.** Local pipeline in `voice/`: openWakeWord with a custom-trained "Hey Athena" model (supersedes the earlier interim "Hey Jarvis" stand-in) for activation, a local SpeechBrain ECAPA-TDNN voiceprint for speaker verification (only the enrolled voice triggers a response, everything else is silently dropped), faster-whisper for STT, Piper for TTS (`en_US-hfc_female-medium` voice), headless `claude -p` (dedicated resumed session) as the bridge into Claude Code. No keypress anywhere in the runtime loop — activation is wake-word + voiceprint only. The wake-word model was trained via openWakeWord's synthetic-TTS pipeline (`voice/training/`, gitignored — 15,000 positive + 15,000 adversarial-negative clips, the full MIT room-impulse-response set, the ACAV100M negative-feature set), validated on a 4,000-clip held-out set at 88.8% recall / 2.7% false-accept rate against deliberately confusable phrases, then confirmed live end-to-end. An earlier build used resemblyzer's GE2E encoder for speaker verification; live testing caught it producing a real false accept (impostor scored *higher* than the genuine owner), so it was replaced with ECAPA-TDNN, a model purpose-built for speaker verification. Live testing of the full loop also surfaced a second real issue: the speaker-verification `accept_threshold` (0.68) was rejecting genuine attempts (a deliberate, focused utterance scored 0.677 and still missed), so it was lowered to 0.60, which still leaves real separation from an impostor (0.73 owner vs. 0.26 impostor in earlier testing). Works identically on Windows and Fedora (`skills/engineering/voice-setup.md` drives setup on either; Fedora not yet tested live). Ultron's edge/gesture/call-UI approach stays a v2 experiment, only worth revisiting once the core loop has run for a while in daily use.
...
Git-initialized with a remote: `github.com/Lykon130/A.T.H.E.N.A`. 23 commits in: initial scaffold, Engineering docs skills, documentation baseline, `document-commit` wired as a pre-push hook, vault folders + Obsidian MCP connection, the local voice module (plus the custom "Hey Athena" wake-word model trained after that), and the desktop HUD — most followed by their own `docs: update for <hash>` commit from the hook.
...
5. ~~Wire voice~~ — done: `voice/` module (wake word + speaker verification + faster-whisper + Piper + headless Claude Code bridge), plus `skills/engineering/voice-setup.md`. Built and tested end-to-end on Windows; Fedora documented in `voice/README.md`, not yet tested live there. Custom "Hey Athena" openWakeWord model (trained, tuned, and validated live end-to-end) supersedes the interim `hey_jarvis` model; the training workspace (`voice/training/` — datasets, cloned repos, dedicated venv) is gitignored as local build state, not source.
...
Steps 1-7 are closed. Open threads, tracked in `docs/SRS.md` §6: Fedora-side voice/hook/Syncthing activation.
